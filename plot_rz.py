...
stepPositionZ = caloHits["AllCaloHitContributionsCombined.stepPosition.z"]
xHits = ak.flatten(stepPositionX.arrays(), axis=None)
yHits = ak.flatten(stepPositionY.arrays(), axis=None)
zHits = ak.flatten(stepPositionZ.arrays(), axis=None)
rHits = np.sqrt(xHits * xHits + yHits * yHits)
